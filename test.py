...
# 定义计算彩色信息特征的函数
def compute_color_feature(inpuy_xyz_image, x, y, z):
    color_feature = np.sum(inpuy_xyz_image[:, :, 0] * x + inpuy_xyz_image[:, :, 1] * y + inpuy_xyz_image[:, :, 2] * z)
...
for filename in os.listdir(good_quality_folder):
    ...
    # 提取彩色信息的特征
    color_features = []
    lambda_range = np.arange(380, 781, 5)
